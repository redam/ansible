...
options:
  ...
  api_passhash:
    description:
      - Passhash of API user (see https://www.paessler.com/manuals/prtg/my_account_settings.htm)
    required: true
  prtg_url:
    description:
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            enabled=dict(default=True, choices=BOOLEANS),
    ...
    if not device_id:
        ...
        check_resp, check_info = api_call(module, '/api/table.json', {'content':'devices', 'output':'json', 'count':1000, 'columns':'objid,device,host,group,active'})
